...
static bool parse_register(const char *token, char expected_prefix, int *reg_num) {
    if (!token || token[0] != expected_prefix) return false;
    char *endptr;
    long val = strtol(token + 1, &endptr, 10);
    if (*endptr != '\0' && !isspace((unsigned char)*endptr)) return false;
    if (expected_prefix == 'x' && (val < 0 || val > 255)) return false;
    if (expected_prefix == 'v' && (val < 0 || val > 31)) return false;
    *reg_num = (int)val;
    return true;
}
...
static int get_branch_opcode(const char *token) {
    if (strcasecmp(token, "BEQ") == 0) return 0x10;
    if (strcasecmp(token, "BNE") == 0) return 0x11;
    if (strcasecmp(token, "BCS") == 0) return 0x12;
    if (strcasecmp(token, "BCC") == 0) return 0x13;
    if (strcasecmp(token, "BMI") == 0) return 0x14;
    if (strcasecmp(token, "BPL") == 0) return 0x15;
    if (strcasecmp(token, "BVS") == 0) return 0x16;
    if (strcasecmp(token, "BVC") == 0) return 0x17;
    if (strcasecmp(token, "BHI") == 0) return 0x18;
    if (strcasecmp(token, "BLS") == 0) return 0x19;
    if (strcasecmp(token, "BGE") == 0) return 0x1A;
    if (strcasecmp(token, "BLT") == 0) return 0x1B;
    if (strcasecmp(token, "BGT") == 0) return 0x1C;
    if (strcasecmp(token, "BLE") == 0) return 0x1D;
    if (strcasecmp(token, "BAL") == 0) return 0x1E;
    return -1;
}
...
bool compile(const char *input_source_file, const char *output_byte_file) {
    if (!input_source_file || !output_byte_file) return false;
    ...
    /* =========================================================================
     * Pass 1: Find Labels and calculate instruction addresses
     * ========================================================================= */
    ...
    for (int i = 0; i < total_lines; i++) {
        ...
        if (!parsed && strncasecmp(trimmed, "Read ", 5) == 0) {
            ...
            for (char *p = clean_read; *p; p++) if (*p == ',') *p = ' ';
        ...
        if (!parsed && strncasecmp(trimmed, "Write ", 6) == 0) {
            ...
            for (char *p = clean_write; *p; p++) if (*p == ',') *p = ' ';
            ...
            if (equals) {
                *equals = '\0';
                ...
                if (!parsed) {
                    ...
                    char *op_pos = NULL;
                    /* Check for +, -, *, / operators */
                    if ((op_pos = strchr(rhs, '+'))) op_char = '+';
                    else if ((op_pos = strchr(rhs, '-'))) op_char = '-';
                    else if ((op_pos = strchr(rhs, '*'))) op_char = '*';
                    else if ((op_pos = strchr(rhs, '/'))) op_char = '/';
                        ...
                        if (is_dest_vec) {
                            ...
                            if (parse_register(op1_str, 'v', &v_src1)) {
                                if (parse_register(op2_str, 'v', &v_src2)) {
                                    /* Vector op Vector */
                                    if (op_char == '+') inst.opcode = 0x21;
                                    else if (op_char == '-') inst.opcode = 0x22;
                                    else if (op_char == '*') inst.opcode = 0x23;
                                    ...
                                    /* Vector op Constant */
                                    if (op_char == '+') inst.opcode = 0x29;
                                    else if (op_char == '-') inst.opcode = 0x2A;
                                    else if (op_char == '*') inst.opcode = 0x2B;
                        ...
                        /* Integer Arithmetic */
                            ...
                            if (parse_register(op1_str, 'x', &x_src1)) {
                                if (parse_register(op2_str, 'x', &x_src2)) {
                                    ...
                                    if (op_char == '+') inst.opcode = 0x01;
                                    else if (op_char == '-') inst.opcode = 0x02;
                                    else if (op_char == '*') inst.opcode = 0x03;
                                    else if (op_char == '/') inst.opcode = 0x04;
                                    ...
                                    /* Integer op Constant */
                                    if (op_char == '+') inst.opcode = 0x09;
                                    else if (op_char == '-') inst.opcode = 0x0A;
                                    else if (op_char == '*') inst.opcode = 0x0B;
                                    else if (op_char == '/') inst.opcode = 0x0C;
